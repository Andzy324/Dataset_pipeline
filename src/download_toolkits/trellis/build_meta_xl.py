...
if __name__ == '__main__':
    ...
    # Future: python dataset_toolkits/build_metadata.py <SUBSET> --output_dir <OUTPUT_DIR> [--source <SOURCE>]
    # python dataset_toolkits/build_metadata.py 3D-FUTURE  --output_dir datasets/3D-FUTURE


    # HSSD: python dataset_toolkits/build_metadata.py HSSD --output_dir datasetsHSSD
    # python dataset_toolkits/build_metadata.py HSSD  --output_dir datasets/HSSD

    # python dataset_toolkits/build_metadata.py ObjaverseXL  --output_dir datasets_filtered/ObjaverseXL_sketchfab
    ...
    # integrate optional pre-download using dataset_utils.download
    parser.add_argument('--auto_download', action='store_true',
                        help='Automatically call dataset download() before merging records')
    parser.add_argument('--rank', type=int, default=0,
                        help='Shard rank for auto download (0..world_size-1)')
    parser.add_argument('--world_size', type=int, default=1,
                        help='Number of shards for auto download')
    ...
    # optional: auto download via trellis pipeline before merging
    if opt.get('auto_download', False):
        os.makedirs(opt.output_dir, exist_ok=True)
        # filter to not-downloaded rows if local_path exists
        md = metadata.reset_index()
        if 'local_path' in md.columns:
            md = md[md['local_path'].isna()]
        # sharding
        total = len(md)
        if total > 0:
            start = total * int(opt.get('rank', 0)) // max(1, int(opt.get('world_size', 1)))
            end = total * (int(opt.get('rank', 0)) + 1) // max(1, int(opt.get('world_size', 1)))
            shard = md.iloc[start:end]
            print(f"Auto downloading shard {opt.get('rank',0)}/{opt.get('world_size',1)}: {len(shard)} objects...")
            try:
                downloaded = dataset_utils.download(shard, **opt)
                downloaded.to_csv(os.path.join(opt.output_dir, f'downloaded_{opt.get("rank",0)}.csv'), index=False)
    ...
    for model in image_models:
        ...
        if f'feature_pc_{model}' not in metadata.columns:
            metadata[f'feature_pc_{model}'] = [False] * len(metadata)
            ...
            def worker(sha256):
                try:
                    ...
                    for model in image_models:
                        ...
                        if need_process(f'feature_pc_{model}') and \
                            metadata.loc[sha256, f'feature_pc_{model}'] == False and \
                            metadata.loc[sha256, 'rendered'] == True and \
                            metadata.loc[sha256, 'voxelized'] == True and \
                            os.path.exists(os.path.join(opt.output_dir, 'feature_models', model, f'{sha256}.npz')):
                            metadata.loc[sha256, f'feature_pc_{model}'] = True
    ...
    with open(os.path.join(opt.output_dir, 'statistics.txt'), 'w') as f:
        ...
        if len(image_models) != 0:
            ...
            f.write(f'  - Number of assets with feature models extracted:\n')
            for model in image_models:
                f.write(f'    - {model}: {metadata[f"feature_pc_{model}"].sum()}\n')
